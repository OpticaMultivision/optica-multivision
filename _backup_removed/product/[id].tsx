...
const ProductDetails: NextPageWithLayout<ProductDetailsProps> = ({
  product,
}) => {
  ...
  const handleCartAmountChange = (quantity: number) => () => {
    const urlWebWhtsap = `https://wa.me/56932271822?text=Hola,%20%20estoy%20interesad@%20en%20consultar%20por%20${item}`;
    window.open(urlWebWhtsap, '_blank');
  };
        ...
        <Box sx={{ '& .styles-module_wrapper__1I_qj': { zIndex: 1201 } }}>
          <FlexContentCenter mb={6}>
            <LazyImage
              ...
              onClick={() =>
                openImageViewer(image.indexOf(image[selectedImage]))
              }
            />
        ...
        <Box mb={3}>
          {price.base > 0 && (
            <H2 color="primary.main" mb={0.5} lineHeight="1">
              ${afterDiscountPrice.toFixed()}
              {price.discount > 0 && (
                <Span color="grey.600" fontSize={18}>
                  {'  '}
                  <del>${price.base.toFixed()}</del>
                </Span>
              )}
            </H2>
          )}
          <Box color={!quantity ? 'red' : 'inherit'}>
            {!quantity ? 'Out of Stock' : 'Stock disponible'}
          </Box>
        </Box>
