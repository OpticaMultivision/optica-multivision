...
const BannerSlider: FC = () => {
  ...
  const { data, loading, refetch } = useFetchSiteSetting(
    `/api/settings/${db_slug.banner_slider}`
  );
  ...
  const deleteOldImage = (image: ImageType) => {
    setOldFiles((state) =>
      state.filter((item) => item.location !== image.location)
    );
  ...
  return (
    <form onSubmit={handleFormSubmit} encType="multipart/form-data">
      <Grid container spacing={3}>
        <Grid item xs={12}>
          <DropZone
            title="Arrastre y suelte la imagen de diapositiva aquí"
            imageSize="subir foto de banner en formato 900px de ancho por 250px de alto"
